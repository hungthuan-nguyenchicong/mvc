...
export default defineConfig({
    ...
    build: {
        ...
        rollupOptions: {
            input: {
                // Định nghĩa các entry point SSR của bạn
                // Mỗi key sẽ là tên của bundle đầu ra (ví dụ: dist/server/main.js, dist/server/admin.js)
                server: './main.js', // Điểm vào chính cho SSR
                //admin: './src/test-socket.js', // Một điểm vào SSR khác (ví dụ cho trang admin)
                // Bạn có thể thêm test-socket.js vào đây nếu nó là một entry point độc lập
                // testSocketEntry: './src/test-socket.js',
            },
            ...
            // main: './src/test-socket.js', // Điểm vào chính cho SSR
            // admin: './src/main.js', // Một điểm vào SSR khác (ví dụ cho trang admin)
            output: {
    ...
    server: {
        ...
        proxy: {
            ...
            '^/(?!src|node_modules|dist|@vite/client|backend)': {
