    ...
    async navigateTo(path) {
        ...
        if (matchedRoute) {
            try {
                ...
                // Use Webpack magic comment to tell it the base directory for dynamic imports
        //const module = await import(/* webpackChunkName: "views" */ `../src/views/${matchedRoute.view || ''}/${matchedRoute.name || ''}`); // Example, adjust based on your `file` structure
        // Or, more generically if `matchedRoute.file` is the full path:
        const module = await import(/* webpackChunkName: "views" */ `${matchedRoute.file}`);
        // 
