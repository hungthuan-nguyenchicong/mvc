// web-mvc/src/admin/core/wysiwyg/wysiwyg.js

function wysiwyg() {
    return render()
}

function render() {
    return /* html */ `
    <div class="toolbar"></div>
    <div id="content" contenteditable="true" style="min-height: 200px">
        <p>&nbsp;</p>
    </div>
    `;
}

function logic() {
    const contentDiv = document.getElementById('content');

    contentDiv.addEventListener('keydown', function(event) {
        if (event.key === 'Enter') {
            event.preventDefault();

            const selection = window.getSelection();
            const range = selection.getRangeAt(0);

            let currentNode = range.startContainer;
            // Nếu startContainer là một text node, lấy node cha của nó
            if (currentNode.nodeType === Node.TEXT_NODE) {
                currentNode = currentNode.parentNode;
            }

            // Tìm thẻ <p> gần nhất
            const currentParagraph = currentNode.closest('p');
            //const previousParagraph = currentParagraph.previousElementSibling;
            if (currentParagraph) {
                //previousParagraph.innerHTML.replace(/&nbsp;/g, '');
                // Tìm thẻ <p> đứng trước đó
                // const previousParagraph = currentParagraph.previousElementSibling;

                // // Kiểm tra xem previousParagraph có tồn tại và có phải là thẻ <p> không
                // if (previousParagraph && previousParagraph.tagName === 'P') {
                //     // Nếu tồn tại, thực hiện thay thế
                //     previousParagraph.innerHTML = previousParagraph.innerHTML.replace(/&nbsp;/g, '');
                // }

                const newParagraph = document.createElement('p');
                newParagraph.innerHTML = '&nbsp;';

                // Tìm thẻ <p> đứng trước đó
                const previousParagraph = currentParagraph.previousElementSibling;

                // Kiểm tra xem previousParagraph có tồn tại và có phải là thẻ <p> không
                if (previousParagraph && previousParagraph.tagName === 'P') {
                    // Nếu tồn tại, thực hiện thay thế
                    previousParagraph.innerHTML = previousParagraph.innerHTML.replace(/&nbsp;/g, '');
                }

                // Chèn thẻ p mới vào sau thẻ p hiện tại
                currentParagraph.after(newParagraph);
                
                // Di chuyển con trỏ vào đầu thẻ p mới
                const newRange = document.createRange();
                newRange.setStart(newParagraph, 1);
                newRange.collapse(true);
                
                selection.removeAllRanges();
                selection.addRange(newRange);
            }
        }
    });
}

export { wysiwyg, logic };