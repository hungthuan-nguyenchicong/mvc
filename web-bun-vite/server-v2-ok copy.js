...
serve({
  ...
  async fetch(request) {
    ...
    if (!isProduction) {
        // Kiểm tra xem đường dẫn có phải là từ thư mục gốc của public không (ví dụ: /vite.svg)
        // Hoặc là từ thư mục src (ví dụ: /src/main.js)
        let filePath;
        if (pathname.startsWith('/src/')) {
            filePath = path.join(projectRoot, pathname);
        } else {
            // Đối với các đường dẫn khác, giả định chúng nằm trong thư mục public
        ...
        try {
            ...
            // Nếu không tìm thấy trong src hoặc public, tiếp tục để xử lý 404 cuối cùng
        }
    }
